...
export default class AssignmentCreation extends LightningElement {
    ...
    let temp = [];
    if (result.data) {
        console.log('Inside GetProjectDetails', JSON.stringify(result.data));
        ...
        console.log('relatedto-----', JSON.stringify(temp));
        this.Related_To__cOptions = temp;
        } else if (result.error) {
        console.error('Error fetching project names:', result.error);
        }
    }
    ...
    // handleInputChangeforChild(event)
    // {
    //     const rowId = event.target.dataset.id;
    ...
    addSubassignmentToParent(parentIndex) {
        // this.Related_To_cOptions_Child = null;
    ...
    // handleSave() {
    //     console.log('line 248', JSON.stringify(this.assignments));
    
    //     if (this.assignments && this.assignments.length > 0) {
    //         const parentAssignments = this.assignments.filter(assignment => !assignment.parentId);
    //         const subassignments = this.assignments.filter(assignment => assignment.parentId);
    ...
    //         //             subassignments: JSON.stringify(subassignments
    ...
    //         //                     parentAssignment:assignment.Id,
    //         //                     parentId:  subassignment.parentId
    //         //                 })),
    //         //             ),
    //         //         })),
    //         //     ),
    ...
    //         //     assignmentWrapper: JSON.stringify(
    //         //         parentAssignments.map(assignment => {
    //         //             // Log filter condition values
    //         //             console.log('Filter Condition:', assignment.Id);
    ...
    //         console.log('line 276',JSON.stringify(assignmentsData));
    
    //         createAssignment({ assignmentWrapper: assignmentsData.assignmentWrapper })
    //             .then((createdAssignments) => {
    //                 console.log('line 281: Inside then block', JSON.stringify(createdAssignments));
    
    //                 if (createdAssignments && createdAssignments.length > 0) {
    //                     this.assignments = createdAssignments;
    //                     console.log("line 277", JSON.stringify(this.assignments));
    //                     return refreshApex(this.wiredAssignments);
    //                 } else {
    //                     console.error('Error: Empty or invalid response from Apex method.');
    //                 }
    //             })
    //             .catch((error) => {
    ...
    //             assignmentWrapper: JSON.stringify(
    //                 parentAssignments.map(assignment => {
    //                     const subassignments = this.assignments
    //                         .filter(subassignment => subassignment.parentId === assignment.Id)
    //                         .map(subassignment => ({
    //                             assignmentName: subassignment.Name,
    //                             assignedTo: subassignment.Assigned_To__c,
    ...
    //             ),
    //         };
    //         console.log('line 371',JSON.stringify(assignmentsData));
    //         createAssignment({ assignmentWrapper: assignmentsData.assignmentWrapper })
    //         .then((createdAssignments) => {
    //             console.log('line 281: Inside then block', JSON.stringify(createdAssignments));
    //             // Update the existing rows with the returned data
    //             for (let i = 0; i < createdAssignments.length; i++) {
    //                 const assignmentIndex = this.findIndexById(createdAssignments[i].Id);
    //                 if (assignmentIndex !== -1) {
    //                     this.assignments[assignmentIndex] = createdAssignments[i];
    //                 }
    //             }
    //             return refreshApex(this.wiredAssignments)
    //             .then(() => {
    //                 // Check if fetchDropdownData is called after refreshing assignments
    //                 console.log('Before fetchDropdownData in handleSave');
    //                 this.fetchDropdownData();
    //                 console.log('After fetchDropdownData in handleSave');
    //             })
    //             .catch((error) => {
